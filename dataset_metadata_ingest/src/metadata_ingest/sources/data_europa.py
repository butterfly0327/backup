...
class DataEuropaCollector(BaseDatasetCollector):
    ...
    def iter_records(self, checkpoint: Dict[str, Any]) -> Iterator[Tuple[NormalizedDatasetRecord, Dict[str, Any]]]:
        ...
        while True:
            ...
            for item in items:
                ...
                try:
                    detail = self.get_json(f"{self.source.base_url}/api/hub/repo/datasets/{source_key}")
                    yield self._normalize(search_item=item, detail_payload=detail, source_key=source_key), {"page": page}
                except Exception:
                    try:
                        yield self._normalize(search_item=item, detail_payload=None, source_key=source_key), {"page": page}
    ...
    def _normalize(
        ...
        *,
        ...
        landing_url = (
            ...
            or self._first_text_any(search_item, ["landingPage", "url", "uri", "id", "about"])
            or f"{self.source.base_url}/en/data/datasets/{source_key}"
        )
        title = self._first_text_any(dataset_node, ["title", "name"]) or self._first_text_any(search_item, ["title", "name"]) or source_key
        description = self._join_texts(dataset_node, ["description"]) or self._join_texts(search_item, ["description", "summary"])
        themes = self._collect_text_any(dataset_node, ["theme"]) or self._collect_text_any(search_item, ["theme"])
        keywords = self._collect_text_any(dataset_node, ["keyword"]) or self._collect_text_any(search_item, ["keyword", "keywords", "tag"])
        ...
        for dist in distributions:
            size_bytes = parse_bytes(self._first_text_any(dist, ["byteSize", "size"])) or parse_int(self._first_text_any(dist, ["byteSize", "size"]))
            if size_bytes:
            ...
            resources.append(
                {
                    ...
                    "download_url": self._first_text_any(dist, ["downloadURL", "accessURL"]),
        ...
        creators = self._extract_agents(dataset_node, graph_index, ["creator", "publisher", "contactPoint"])
        publisher_name = self._agent_name(creators, "publisher") or self._agent_name(creators, None)
        ...
        if access_value:
            ...
            if any(token in lowered for token in ["restricted", "non-public", "nonpublic", "private"]):
        ...
        return NormalizedDatasetRecord(
            ...
            tags=tags,
            languages=languages,
            license_name=self._first_text_any(dataset_node, ["license", "rights"]) or (licenses[0] if licenses else None),
            license_url=self._first_url_any(dataset_node, ["license"]) or self._first_url_any(search_item, ["license"]),
            access_type=access_type,
            login_required=login_required,
            approval_required=approval_required,
            payment_required=payment_required,
            is_restricted=is_restricted,
            source_created_at=parse_datetime(self._first_text_any(dataset_node, ["issued", "created"])),
            source_updated_at=parse_datetime(self._first_text_any(dataset_node, ["modified"])),
            source_version=self._first_text_any(dataset_node, ["versionInfo", "version"]),
            row_count=parse_int(self._first_text_any(dataset_node, ["recordCount", "rows"])),
            ...
            schema_json={
                ...
                "temporal_resolution": self._first_text_any(dataset_node, ["temporalResolution"]),
                "spatial_resolution_meters": self._first_text_any(dataset_node, ["spatialResolutionInMeters"]),
            },
            metrics_json={
    ...
    def _extract_items(self, payload: Dict[str, Any]) -> List[Dict[str, Any]]:
        candidates = [
            ...
            (payload.get("hits") or {}).get("hits") if isinstance(payload.get("hits"), dict) else None,
        ]
        ...
        for key in ["id", "datasetId", "identifier", "uri", "about", "href"]:
            ...
            for candidate in values:
                if isinstance(candidate, dict):
                    candidate = candidate.get("@id") or candidate.get("id") or candidate.get("identifier")
    ...
    def _extract_dataset_node(
        ...
        if isinstance(detail_payload, list):
            ...
            if isinstance(detail_payload.get("@graph"), list):
                nodes = [node for node in detail_payload.get("@graph") if isinstance(node, dict)]
            else:
    ...
    def _collect_text_any(self, node: Dict[str, Any], local_names: List[str]) -> List[str]:
        values: List[str] = []
        for key, value in node.items():
            ...
            for item in ensure_list(value):
                ...
                if isinstance(item, dict):
                    text = jsonld_first(item, ["prefLabel", "label", "name", "title", "@id"])
                else:
                    text = item
    ...
    def _join_texts(self, node: Dict[str, Any], local_names: List[str]) -> Optional[str]:
        values = self._collect_text_any(node, local_names)
        return "\n\n".join(values) if values else None

    def _first_text_any(self, node: Dict[str, Any], local_names: List[str]) -> Optional[str]:
        values = self._collect_text_any(node, local_names)
        return values[0] if values else None

    def _first_url_any(self, node: Dict[str, Any], local_names: List[str]) -> Optional[str]:
        for key, value in node.items():
            if self._key_local(str(key)) not in local_names:
                continue
            for item in ensure_list(value):
                item = jsonld_value(item)
                if isinstance(item, dict):
                    text = clean_text(item.get("@id") or item.get("id") or item.get("url"))
                else:
    ...
    def _resolve_node(self, value: Any, index: Dict[str, Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    def _extract_distributions(self, dataset_node: Dict[str, Any], index: Dict[str, Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _extract_agents(self, dataset_node: Dict[str, Any], index: Dict[str, Dict[str, Any]], local_names: List[str]) -> List[Dict[str, Any]]:
        ...
        for local_name in local_names:
            ...
            for item in ensure_list(value):
                ...
                agents.append(
                    {
                        "role": local_name,
                        "name": self._first_text_any(node, ["name", "title", "prefLabel"]),
                        "email": self._first_text_any(node, ["email", "mbox"]),
                        "url": self._first_url_any(node, ["homepage", "url"]),
                    }
                )
        return [agent for agent in agents if agent.get("name") or agent.get("email")]

    def _agent_name(self, creators: List[Dict[str, Any]], role: Optional[str]) -> Optional[str]:
