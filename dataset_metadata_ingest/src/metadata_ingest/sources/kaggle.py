...
class KaggleCollector(BaseDatasetCollector):
    ...
    def _normalize(
        ...
        total_size = (
            parse_int(detail.get("totalBytes") or item_dict.get("totalBytes")) or 0
        )
        if not total_size:
            for file in files:
                size_bytes = parse_int(
                    file.get("totalBytes") or file.get("size") or file.get("sizeBytes")
                )
                if size_bytes:
        ...
        metrics_json = {
            ...
            "usability_rating": parse_float(
                detail.get("usabilityRating") or item_dict.get("usabilityRating")
            ),
        }
